...
class Tournament extends React.Component {
	...
	render() {
		...
		var x = Array(31).fill("test", 0, 31);
		var test_matches = x.map((name, index) =>
			<DemoMatch key={index}/>
		);
...
class DemoMatch extends React.Component {
  render() {
    return (
      <div>
        <FormControl component="fieldset" className="demoMatch">
          <RadioGroup
            value="Person A"
          >
            <FormControlLabel value="Person A" disabled={true} control={<Radio />} label="Person A"/>
            <FormControlLabel value="Person B" disabled={true} control={<Radio />} label="Person B"/>
          </RadioGroup>
        </FormControl>
      </div>
    );
